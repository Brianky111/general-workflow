...
POLICY_ANCHORS: dict[str, tuple[str, ...]] = {
    "SKILL.md": (
        ...
        "Before feature similarity triage, run the solution candidate gate",
        "A shared name stem or platform suffix such as Android, iOS, web, or desktop is not merge evidence",
        "each batch directory must separate stable `00-施工.md` from its own mutable `99-进度.md`",
        "Batch progress owns batch-local work/TOS/evidence; root progress only projects batch transitions",
        ...
        "updates its batch-local `99-进度.md` at meaningful work/evidence checkpoints and its root `99-进度.md` only for batch/dependency transitions",
    ...
    "00-progress-router.md": (
        ...
        "## Positive READY Gate",
        "a minimal solution frame identifies each owned contribution, construction-stage ledger, and aggregate proof",
        "before feature similarity triage, whether the request describes one independently acceptable feature",
        "run the solution candidate gate",
        "a stable construction source and separate progress source for every batch",
        "Batch-local and root status own different scopes",
    ...
    "00-solution-framing.md": (
        "Frame a finite, goal-bounded delivery that needs several independently acceptable feature contributions",
        "## Solution Candidate Gate",
        "Run this gate before feature similarity triage",
        "A shared repository/product stem or a platform suffix",
        "## Durable Staged Construction Surface",
        "Treat each batch's `99-进度.md` as the sole mutable authority for that batch",
        "Treat the root `99-进度.md` as the aggregate authority only",
        "## Existing Flat-Batch Migration",
        "do not leave an active solution with mixed flat-file and directory batch schemas",
        "never retain flat and directory forms as two editable truths",
    ...
    "00-business-taxonomy.md": (
        ...
        "Before feature similarity triage, run the solution candidate gate",
        "A solution batch is a construction/coordination stage",
        "each batch own a stable `00-施工.md` plus a separate batch-local `99-进度.md`",
    ...
    "02-requirements-capture.md": (
        ...
        "Before classifying a similar name as New/Merge/Revision, run the solution candidate gate",
        "Treat names only as discovery hints",
        "Never merge the aggregate request itself into a feature contract",
    ...
    "99-status-and-evidence.md": (
        ...
        "It is the only manually maintained status for that batch",
        "the only manually maintained aggregate status",
        "Use counts as the default progress record at both scopes",
        "The root solution status owns only batch transitions",
        "the root cannot replace batch-local progress",
        "Do not update status after every internal workflow stage",
        "Do not create or hand-maintain equivalent `workflow-state.json`, feature `status.json`, and `99-进度.md` mirrors in parallel",
        "choose one manual authority",
        "other views at that same scope must be generated or treated as non-authoritative",
