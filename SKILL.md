...
description: Guide agent-led software work through a staged, document-governed general development workflow with progress detection and on-demand reference loading. Use when Codex needs to pick up an existing feature workflow, create requirements/contracts/plans/tests, continue TDD implementation, run verification, or handle workflow changes without reading a full process manual up front.
---
