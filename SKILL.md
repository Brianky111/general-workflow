...
description: Guide agent-led software delivery through an implementation-forward, evidence-driven workflow with progress detection, goal-bounded cross-feature solution framing, vertical feature slicing, concise BDD contracts, reuse-aware planning, code-bound TDD, and proportionate verification. Use when kicking off a project, framing a multi-feature solution, decomposing modules/features/use cases, defining Given-When-Then behavior, creating or revising requirements/contracts/plans/tests, extending an existing codebase, connecting UI/API/domain/infrastructure, continuing implementation, recertifying refactors, verifying cross-feature or E2E behavior, or handling changes. Typical triggers include 新项目开工, 总体方案, 功能拆分, BDD, TDD, 接手/继续开发, 重构/整理, verify, and change requests.
---
...
- Keep the stable module/feature ownership hierarchy separate from a goal-bounded solution delivery view. When one accepted aggregate outcome needs several independently owned features/modules or aggregate integration/release proof, read `00-solution-framing.md`, record only the minimal solution outcome/non-goals, feature-owner map, dependency order, and aggregate proof, then execute one owning feature gap at a time. A solution references feature truth; it never copies requirements, turns a module into a temporary solution, or makes change rounds into child features.
...
- `00-solution-framing.md`: coordinate a finite cross-feature solution without duplicating feature behavior truth or confusing stable modules, features, and change rounds.
